...
def harmonic_tolerance_db(k, base_tolerance_db, crossover=5, extra_db=2.0):
    """
    Return the amplitude tolerance (dB) for harmonic number `k`.

    Harmonics above `crossover` receive an additional `extra_db` of headroom.
    This reflects the physical reality that truncation error has a roughly
    flat absolute noise floor, so its dB impact grows as higher harmonics
    become progressively weaker.

    Parameters
    ----------
    k               : harmonic number
    base_tolerance_db : tolerance applied to harmonics 1..crossover
    crossover       : harmonics above this number get the wider tolerance
    extra_db        : additional dB added beyond the crossover point
    """
    return base_tolerance_db if k <= crossover else base_tolerance_db + extra_db
...
def verify_harmonics(samples, fundamental_hz, wave_type,
                     ...
                     truncation_null_threshold_db=-20.0,
                     tolerance_crossover=5,
                     tolerance_extra_db=2.0):
    """
    Verify the harmonic content of `samples` against the theoretical spectrum
    for `wave_type`.

    Truncation nulls are detected using an absolute threshold: if a harmonic's
    measured level is below `truncation_null_threshold_db` (relative to the
    fundamental) AND it exceeds its per-harmonic tolerance, it is considered
    to be in a truncation null and is skipped.  This is more robust than a
    floor estimator, which can be dragged down by the nulls themselves.

    The pass/fail tolerance widens for harmonics above `tolerance_crossover`
    by `tolerance_extra_db` dB, reflecting the fact that truncation error has
    a roughly flat absolute noise floor whose dB impact grows as higher
    harmonics become progressively weaker.

    Parameters
    ----------
    amplitude_tolerance_db      : base pass/fail tolerance for harmonics 1..crossover
    absence_threshold_db        : harmonics that should be absent must be below this
    truncation_null_threshold_db: harmonics measured below this level (dB re
                                  fundamental) are treated as truncation nulls
                                  and skipped when they would otherwise fail
    tolerance_crossover         : harmonics above this number use the wider tolerance
    tolerance_extra_db          : extra dB added to the tolerance above the crossover
    ...
    for k, rel_amp in present_expected.items():
        ...
        tol_db = harmonic_tolerance_db(
            k, amplitude_tolerance_db, tolerance_crossover, tolerance_extra_db)

        error_db = abs(meas_db - exp_db)

        # A harmonic is in a truncation null when it would fail on tolerance
        # AND its absolute level has dropped below the null threshold.
        # Using an absolute threshold avoids the floor-estimator being dragged
        # down by the very nulls it is trying to detect.
        in_truncation_null = (
            error_db >= tol_db and
            meas_db < truncation_null_threshold_db
        )
...
def plot_amplitudes(wave_type, amplitudes):
    ...
    for freq_str, vals in amplitudes.items():
        ...
        try:
            ...
            present, absent, passed = verify_harmonics(
                norm, fundamental_hz, wave_type)
