...
def verify_harmonics(samples, fundamental_hz, wave_type,
    ...
    Parameters
    ----------
    amplitude_tolerance_db : base pass/fail tolerance for present harmonics
    absence_threshold_db   : harmonics that should be absent must be below this
    noise_floor_db         : harmonics expected below this level are skipped —
                             phase accumulator truncation noise dominates there
    rolling_tolerance      : if True, widens tolerance for weaker harmonics to
                             account for the constant-power truncation noise floor
                             (its dB impact grows as the harmonic gets quieter)

    Returns
    -------
    present_results : dict  harmonic_n → {expected_db, measured_db, error_db,
                                          tolerance_db, passed, skipped}
    absent_results  : dict  harmonic_n → {measured_db, passed}
    overall_pass    : bool
    """
    n      = len(samples)
    window = np.hanning(n)
    ...
    for k, rel_amp in present_expected.items():
        ...
        # Skip harmonics that are theoretically below the noise floor —
        # truncation error dominates and a failure here is meaningless
        if exp_db < noise_floor_db:
            present_results[k] = dict(
                ...
                error_db     = None,
                tolerance_db = None,
                passed       = True,   # not checked, so doesn't fail
                skipped      = True,
            )
            continue
        ...
        # Widen the tolerance for weaker harmonics: truncation noise is
        # constant power, so its dB impact grows as the harmonic gets quieter.
        # Extra slack kicks in below −12 dB, scaling at 0.5 dB per dB.
        if rolling_tolerance:
            tolerance = amplitude_tolerance_db + max(0.0, -exp_db - 12.0) * 0.5
        else:
            tolerance = amplitude_tolerance_db
