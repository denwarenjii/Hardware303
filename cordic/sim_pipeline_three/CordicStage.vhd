...
--    21 Feb 25     Chris M.   Changed i from input to generic.
--    25 Feb 25     Chris M.   Added documentation.
--
-- TODO:
--  - Internally generate the d signal.
--
-------------------------------------------------------------------------------
library ieee;
use std.textio.all;
...
package CordicConstants is
 ...
 -- we repeat iterations 4 and 13 so that the expression
  ...
  -- String arrays for debugging using value as index.
...
-- CordicStage entity declaration.
--
-- This entity describes a stage of a fully parallel cordic. A stage consists
-- of X, Y, and Z adders, and has a fixed right shift amount determined at compile time.
-- Note that only one input (the A input) per adder is needed, since the B for either the X
-- or Y adder is the A input for the opposite adder shifted right i times. Finally, note that
-- iteration repititions do assure convergence for hyperbolic functions is only valid for i < 40.
-- See Walther (1971) for more info.
--
-- Generics:
--
--    wordsize : The size of the inputs.
--    i        : The index of this CordicStage. Determines the shift amounts and iteration 
--                 reptitions.
-- Inputs:
--
--    XIn        : The A input for the X adder. X >> i is the B input for the Y adder.
--    YIn        : The B input for the Y adder. Y >> i is the B input for the X adder.
--    ZIn        : The A input for the Z adder.
--    ZConstant  : The B input for the Z adder.
--    CordSystem : The coordinate system this stage is operating in (Circular, Linear, or 
--                   Hyperbolic). Defined in the CordicConstants package.
--    d          : The decision variable for this adder. Used to generate the Add/BarSub signals
--                   for each adder. Note that because these only depend on the inputs to the 
--                   current stage, they can be generated internally (and should be later).
--
-- Outputs:
--
--    XOut       : The output of the X adder.
--    YOut       : The output of the Y adder.
--    ZOut       : The output of the Z adder.
--
entity CordicStage is
  generic (
    wordsize : positive := 22;
    i        : positive range 0 to wordsize
  );
...
architecture concurrent of CordicStage is
  ...
  -- Used the pass the X Input through when the coordinate system is linear.
  signal XOutMux : std_logic_vector(XIn'range);

  -- `m` as shown in Walther's paper. The Cordinate System constants were picked
  ---  such that the high bit corresponds to it's sign.
...
begin
  ...
  -- For 0 <= i <= 15
  --
  -- The iteration sequence is as follows for hyperbolic functions:
  --
  --  0 -> 1 (+ 1)
  --  1 -> 2
  --  2 -> 3
  ...
  --  14 -> 13 ( - 1)
  --  15 -> 14
  X_BIn <= std_logic_vector( shift_right( signed(Yin), (i + 1) ) ) when  ((CordSystem = HYPERBOLIC) and (i < 4))  else
           std_logic_vector( shift_right( signed(Yin), (i - 1) ) ) when  ((CordSystem = HYPERBOLIC) and (i > 13)) else
           std_logic_vector( shift_right( signed(Yin), i));

  Y_BIn <= std_logic_vector( shift_right( signed(Xin), (i + 1) ) ) when  ((CordSystem = HYPERBOLIC) and (i < 4))  else
           std_logic_vector( shift_right( signed(Xin), (i - 1) ) ) when  ((CordSystem = HYPERBOLIC) and (i > 13)) else
           std_logic_vector( shift_right( signed(Xin), i));
  ...
  XAdder : entity AddSub
    ...
    port map (
      ...
      Cout        => open,        -- Carry out is unused. Might cause warnings in Xilinx ISE.
      S           => XOutMux
    );
