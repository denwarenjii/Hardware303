...
--    22 Feb 2025   Chris M.   Modified test bench for pipelined design.
--    25 Feb 2025   Chris M.   Modified test bench to allow for 3 pipeline stages.
--
--  TODO:
...
architecture TestBench of Cordic_TB is
  ...
  -- A 5-tuple of 7 char strings
  type String5Tup is array(0 to 4) of string(1 to 7);
  ...
  type Int5Tup is array(0 to 4) of SignedWordInt;
...
begin
  ...
  RunTest : process(Clk_TB)
    ...
    -- The past 5 functions, X, and Y as unsigned integers.
    variable PrevFuncSync : Int5Tup := ( 0, 0, 0 ,0, 0) ;
    variable PrevXSync    : Int5Tup := ( 0, 0, 0 ,0, 0);
    variable PrevYSync    : Int5Tup := ( 0, 0, 0 ,0, 0);

    -- The past 5 functions as strings.
    variable PrevFuncStringSync     : String5Tup := 
      ( (others => ' '), (others => ' '), (others => ' '), (others => ' '), (others => ' ') );

    -- The past 5 results as unsigned integers.
    variable PrevExpectedResultSync : Int5Tup := ( 0, 0, 0, 0, 0 );
  ...
  begin
    ...
    if (not Seeded) then
      ...
      -- Skip the first 5 checks while we wait for the results to come in.
      if (EdgeCount < 5) then
        ...
        Diff  := Q1_14ToReal(R_TB) - Q1_14ToReal(PrevExpectedResultSync(LATENCY));
        Cond  := Diff <= PRECISION;
        ...
        Log(          "PrevFuncStringSync(4): " & PrevFuncStringSync(4) , DEBUG);
        Log(          "PrevFuncStringSync(3): " & PrevFuncStringSync(3) , DEBUG);
        ...
        Log(to_string(LF), DEBUG);
        Log(          "PrevXSync(4): "           & to_string(Q1_14ToReal(PrevXSync(4))) , DEBUG);
        Log(          "PrevXSync(3): "           & to_string(Q1_14ToReal(PrevXSync(3))) , DEBUG);
        ...
        Log(to_string(LF), DEBUG);
        Log(          "PrevYSync(4): "           & to_string(Q1_14ToReal(PrevYSync(4))) , DEBUG);
        Log(          "PrevYSync(3): "           & to_string(Q1_14ToReal(PrevYSync(3))) , DEBUG);
        ...
        AffirmIf (
          ...
          -- Received message
          Color & 
          PrevFuncStringSync(LATENCY) & 
          " X: " & to_string(Q1_14ToReal(PrevXSync(LATENCY)), 6) & 
          " ("   & to_string(int_to_slv(PrevXSync(LATENCY), WORD_SIZE)) & ") " &
          " Y: " & to_string(Q1_14ToReal(PrevYSync(LATENCY)), 6) & " " &
          " ("   & to_string(int_to_slv(PrevYSync(LATENCY), WORD_SIZE)) & ") " &
          HT & "Received: " & to_string (Q1_14ToReal(R_TB), 6) & " ( " & to_string(R_TB) & " ) " & ANSI_RESET,

          -- Expected message
          HT & Color & "PrevExpectedResult(" & to_string(LATENCY) & ") is " & 
          to_string(Q1_14ToReal(PrevExpectedResultSync(LATENCY)), 6) &
          " (" & to_string(int_to_slv(PrevExpectedResultSync(LATENCY), WORD_SIZE)) & ")" 
          & ANSI_RESET & 
          " Diff: " & to_string(Diff)
        );
      end if;
            ...
            case uint_to_slv(TmpFunc, Func_TB'length) is
              ...
              when F_COS_X    =>
                ...
                then
                  ...
                  PrevFuncSync(1 to 4) := PrevFuncSync(0 to 3);
                  PrevFuncSync(0)      := CurrentFunc;
                  CurrentFunc          := TmpFunc;
                  
                  PrevXSync(1 to 4)    := PrevXSync(0 to 3);
                  PrevXSync(0)         := CurrentX;
                  CurrentX             := TmpX;
                  
                  PrevYSync(1 to 4)    := PrevYSync(0 to 3);
                  PrevYSync(0)         := CurrentY;
                  
                  PrevFuncStringSync(1 to 4) := PrevFuncStringSync(0 to 3);
                  PrevFuncStringSync(0)      := CurrentFuncString;
                  CurrentFuncString          := "cos(x) ";
                  
                  PrevExpectedResultSync(1 to 4) := PrevExpectedResultSync(0 to 3);
