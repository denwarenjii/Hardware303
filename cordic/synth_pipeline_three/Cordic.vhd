...
--    28 May 2026   Chris M.   Added tanh support
-- TODO:
...
entity Cordic is
  port(
    ...
    Func  : in  std_logic_vector( 4 downto 0); -- Function to perform
...
begin
  ...
  XInputBus(4) <= XPipeline1;
  YInputBus(4) <= YPipeline1;
  ZInputBus(4) <= ZPipeline1;
  ...
  XInputBus(8) <= XPipeline2;
  YInputBus(8) <= YPipeline2;
  ZInputBus(8) <= ZPipeline2;

  InputBusGen_ThirdQuarter :
  for i in 9 to 11 generate 
    XInputBus(i) <= XResultBus(i-1);
    YInputBus(i) <= YResultBus(i-1);
    ZInputBus(i) <= ZResultBus(i-1);
  end generate;
  ...
  XInputBus(12) <= XPipeline3;
  YInputBus(12) <= YPipeline3;
  ZInputBus(12) <= ZPipeline3;
